...
#SERVICE_USER="${SERVICE_USER:-proftpd}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-proftpd}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    if [ "$ENTRYPOINT_MESSAGE" = "yes" ]; then
      echo "Executing entrypoint script for proftpd"
    fi
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
